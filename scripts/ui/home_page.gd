extends Control
## Home summary page for the offline prototype.
...
func _apply_styles() -> void:
	...
	UiStyle.apply_body_label(_display_name_label)
	UiStyle.apply_body_label(_region_label)
	UiStyle.apply_body_label(_needed_count_label)
	UiStyle.apply_panel(_summary_panel)
...
func _refresh() -> void:
	_disclaimer_label.text = (
		"Independent community companion. This app never logs into Pokémon GO, "
		+ "never asks for game passwords, and never automates gifts, friendship, or trades."
	)

	if AppData.has_saved_profile():
		var profile := AppData.profile
		_display_name_label.text = "Display name: %s" % profile.display_name
		_region_label.text = "Postcard region: %s" % profile.region
		_needed_count_label.text = "Needed regions selected: %d" % profile.needed_regions.size()
		_setup_button.visible = false
		_summary_panel.visible = true
	else:
		_display_name_label.text = "No local profile saved yet."
		_region_label.text = "Create a profile to track your postcard region."
		_needed_count_label.text = "Needed regions selected: 0"
